...
public class ExecJavaMojo extends AbstractExecMojo {
                ...
                () -> {
                    ...
                    try {
                        if (sepIndex >= 0) {
                            ...
                            doExecModuleLayer(moduleName, bootClassName);
                        } else {
    ...
    private void doExecClassLoader(final String bootClassName) throws Throwable {
        Class<?> bootClass = Thread.currentThread().getContextClassLoader().loadClass(bootClassName);
        executeMainMethod(bootClass);
    }

    private void doExecModuleLayer(final String moduleName, final String bootClassName) throws Throwable {
    ...
    private void executeMainMethodForModule(Class<?> bootClass) throws Throwable {
        // The package has already been opened by the controller in doExecModuleLayer
        // so we can now access the main method via reflection
        try {
    ...
    private void executeMainMethod(Class<?> bootClass) throws Throwable {
